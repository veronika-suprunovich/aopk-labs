...
struct_deskriptor  struc  
	...
	;бит 4-тип дескриптора(0-системный,1-обычный)
	;бит 5-6 --уровень привилегий дескриптора(DPL)
	;бит 7-бит присутствия сегмента
    attribs      db 0     ;Лимит сегмента биты 3-0 (19:16) и атрибуты
	;атрибуты:
	;бит 4-зарезервирован для операционной системы
	;бит 5 = 0
	;бит 6- бит разрядности( 0 это 16-битный сегмент, 1 это 32-битный сегмент)
	;бит 7 -бит гранулярности(0-лимит в байтах, 1 лимит в 4-килобайтных единицах)
    base_hight   db 0     ;Адрес базы, старшая часть  (24:31)
...
;Открываем линию A20 - открытие вентиля для дальнейшей работы c 32 битной памятью(для защищенного режима)
    in  al,92h                        ; За вентиль А20 отвечает 1 бит порта 92h                                                 
...
;Заполняем глобальную таблицу дескрипторов            
    ...
    xor dh,dh       
	;пример 
				;в ax  первоначально                ax =  12345678 abcdefgh
				;записываем в dl старшую часть ax   dx =  00000000 12345678
				;сдвигаем ах на 4 влево 			ax =  5678abcd efgh0000
				;сдвигаем dx на 4 вправо            dx =  00000000 00001234
	;в dl:ax лежит 24-разрядный физ.адрес сегмента dl:ax= 00001234 5678abcd efgh0000
...
input proc near ;Процедура выбора клавиши для переключения режимов
	...
	xor al, al	;ожидаем нажатия еще одной клавиши					   
    int 21h
return:
    	ret
input endp 
